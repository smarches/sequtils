/*  bioio.hpp -- FASTA/Q I/O
 ...
 https://github.com/dancooke/bioio/
 */
...
// stringview
// concepts
// [x] constexpr not magic number
// std::move is a code smell??
// create a simple class to reduce the repetitive template boilerplate in overloads
// std::filesystem for file paths?
// std::remove_if + erase to chop out newlines in fasta records - done some places already
...
namespace bioio {
    ...
    inline std::vector<std::string> split(const std::string& str, const char delim) {
        ...
        const auto n = std::count(std::cbegin(str),std::cend(str),delim);
        result.reserve(n);
        while (std::getline(ss, item, delim)) result.emplace_back(item);
        return result;
    }

    constexpr size_t default_reserve = 100;
...
namespace detail {
    ...
    read_fasta_record(std::istream& fasta)
    {
        ...
        // TODO: first seek the next delim, then count '\n' chars in between so we know the size up front
        // then no futzing with reserve, etc.
        if (!fasta.good() || fasta.peek() == fasta_delim) {
            ...
            SequenceType seq {};
            seq.reserve(line_size * ::bioio::detail::default_reserve);
            const auto line_begin = std::cbegin(line), line_end = std::cend(line);
...
{
    std::vector<std::string> result {};
    result.reserve(::bioio::detail::default_reserve);
    std::string line;
...
// read the reads but not headers
...
// 'default' unaryop (do nothing) that is probably almost always the one used
template <typename StringType = std::string, typename SequenceType = std::string>
FastaReads<StringType, SequenceType> read_fasta_map(const std::string& fasta_path)
{
...
// only read in names from a predefined list
template <typename StringType = std::string, typename SequenceType = std::string,
          typename UnaryOperation>
...
template <typename SequenceType = std::string>
std::vector<SequenceType> read_fastq_seqs(const std::string& path) {
